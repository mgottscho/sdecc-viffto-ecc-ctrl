...
function [decoded_message, num_error_bits] = hamDec(received_codeword, H)
    ...
    if s == zeros(r,1)
        decoded_message = received_codeword; % Assume code is systematic, meaning that first k bit positions are the message, and parity bits are glued on the end
        num_error_bits = 0;
        return;
    end
    ...
    if n == 72 || n == 39
       ...
       if mod(sum(s),2) == 1         
          ...
          if notfound == 1
             ...
             %error = repmat('0',1,n);
             %error(bit) = '1';
             decoded_codeword = bitxor(received_codeword, error);
