...
NUM_CACHELINES=-1 # All
K=64
WORDS_PER_BLOCK=8
NUM_THREADS=$(cat /proc/cpuinfo | grep ^processor | wc -l ) 
SYMBOL_SIZE=8

OUTPUT_DIRECTORY=$MWG_DATA_PATH/swd_ecc_data/$ISA/entropy
...
for SPEC_BENCHMARK in $SPEC_BENCHMARKS; do
    ...
    OUTPUT_FILE="$OUTPUT_DIRECTORY/${ISA}-${SPEC_BENCHMARK}-entropy.mat"
    JOB_STDOUT=$OUTPUT_DIRECTORY/${ISA}-${SPEC_BENCHMARK}-entropy.stdout
    JOB_STDERR=$OUTPUT_DIRECTORY/${ISA}-${SPEC_BENCHMARK}-entropy.stderr
