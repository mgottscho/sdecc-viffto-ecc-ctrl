...
for SPEC_BENCHMARK in $SPEC_BENCHMARKS; do
	...
	./analyze_rv64g_program_statistics_wrapper.sh $PWD $SPEC_BENCHMARK $K $INPUT_FILE $OUTPUT_FILE > $OUTPUT_DIRECTORY/${SPEC_BENCHMARK}-program-statistics.stdout 2> $OUTPUT_DIRECTORY/${SPEC_BENCHMARK}-program-statistics.stderr &
done
