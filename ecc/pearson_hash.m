...
for cur_symbol = 1:total_symbols
    h = my_de2bi(h,8); %convert h to binary vector
    idx = my_bitxor(cc(8*(cur_symbol-1)+1:8*(cur_symbol))+'0',h+'0')-'0'; %this is just bit-wise xor of binary vectors
    h = T(my_bi2de(idx,'left-msb')+1);
end
