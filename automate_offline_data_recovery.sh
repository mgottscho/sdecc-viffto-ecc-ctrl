...
for BENCHMARK in $BENCHMARKS; do
	echo "$BENCHMARK..."
    if [[ "$INPUT_TYPE" == "static" ]]; then # Static evaluation
        INPUT_FILE="$INPUT_DIRECTORY/${ISA}-${BENCHMARK}-data.txt" # For static analysis
        # TODO
    elif [[ "$INPUT_TYPE" == "dynamic" ]]; then # Dynamic evaluation
        INPUT_FILE="$INPUT_DIRECTORY/spike_mem_data_trace_${BENCHMARK}.txt.data.reads" # For dynamic analysis
        #INPUT_FILE="$INPUT_DIRECTORY/spike_mem_data_trace_${BENCHMARK}.txt.data.reads.floats" # For dynamic analysis
    fi
    
    OUTPUT_FILE="$OUTPUT_DIRECTORY/${ISA}-${BENCHMARK}-data-heuristic-recovery.mat"
    JOB_STDOUT=$OUTPUT_DIRECTORY/${ISA}-${BENCHMARK}-data-heuristic-recovery.stdout
    JOB_STDERR=$OUTPUT_DIRECTORY/${ISA}-${BENCHMARK}-data-heuristic-recovery.stderr

#    if [[ "$MWG_MACHINE_NAME" == "hoffman" ]]; then
#        JOB_NAME="swdecc_datarecov_${BENCHMARK}"
#        qsub -V -N $JOB_NAME -l h_data=$MAX_MEM_PER_RUN,time=$MAX_TIME_PER_RUN,highp -M $MAILING_LIST -o $JOB_STDOUT -e $JOB_STDERR -m as -pe shared $NUM_THREADS swd_ecc_offline_data_heuristic_recovery_wrapper.sh $PWD $ISA $BENCHMARK $N $K $NUM_WORDS $NUM_SAMPLED_ERROR_PATTERNS $WORDS_PER_BLOCK $INPUT_FILE $OUTPUT_FILE $NUM_THREADS $CODE_TYPE $POLICY $VERBOSE_RECOVERY
#    elif [[ "$MWG_MACHINE_NAME" == "nanocad-server-testbed" ]]; then
        ./swd_ecc_offline_data_heuristic_recovery_wrapper.sh $PWD $ISA $BENCHMARK $N $K $NUM_WORDS $NUM_SAMPLED_ERROR_PATTERNS $WORDS_PER_BLOCK $INPUT_FILE $OUTPUT_FILE $NUM_THREADS $CODE_TYPE $POLICY $CRASH_THRESHOLD $VERBOSE_RECOVERY $FILE_VERSION $HASH_MODE > $JOB_STDOUT 2> $JOB_STDERR
#    fi
done
